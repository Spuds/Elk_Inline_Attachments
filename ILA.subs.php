...
{
	global $modSettings, $context, $ila_attachments, $txt, $topic, $attachments;
	...
	elseif ($id_msg != -1)
	{
		...
		// save what might have been passed (ah portals)
	...
	if (preg_match_all('~\[attach\s*?(.*?(?:".+?")?.*?|.*?)\][\r\n]?~i', $message, $ila_tags))
	{
		...
		// If they have no permissions to view attachments then we sub out the tag
		// with the appropriate so 'sorry message'
		...
		else
		{
			...
			elseif (!empty($ila_tags))
			{
				...
				//
				// - the tags in the message but there is no attachment, perhaps the attachment
				// did not upload correctly
				// - maybe the user put the tag in wrong because they are rock dumb and did not read our
				// fantastic help, just kidding, really the help is not that good.
				foreach ($ila_tags[1] as $id => $ila_replace)
...
function ila_parse_bbc_tag($data, $attachments, $id_msg, $ila_num, $ila_new_msg_preview)
{
	...
	// Remove this attach choice since we have used it
		$ila_attachments = array_diff($ila_attachments, array($done['id']));
	else
	{
		// Take the first un-used attach number and use it
		$done['id'] = array_shift($ila_attachments);

		// Stick it back on the end in case we need to loop around
		array_push($ila_attachments, $done['id']);
	}
...
function ila_hide_bbc(&$message, $hide_tags = '')
{
	...
	// so we don't try to render them later
	foreach ($hide_tags as $tag)
	{
		// 5.3 here shhhh
		if (stripos($message, '[' . $tag . ']') !== false)
		{
			$message = preg_replace_callback('~\[' . $tag . ']((?>[^[]|\[(?!/?' . $tag . ']))+?)\[/' . $tag . ']~i', function($matches) use($tag) {
...
function ila_load_attachments($msg_id, $topic, $board)
{
	...
	if (!empty($modSettings['attachmentEnable']) && allowedTo('view_attachments', $board) && $topic != -1)
	{
		...
		// Sort the attachments on the id_attach, we need them in order
		$temp = array();
...
function ila_get_topic($msg_id)
{
	...
	if (!empty($msg_id))
	{
		...
		if ($db->num_rows($request) != 1)
			...
			$topic_and_board = $db->fetch_row($request);
